...
regex_setup(const char *regex_str, const char *filename, pcre2_code **re,
    ...
    *re = pcre2_compile((PCRE2_SPTR) regex_str, PCRE2_ZERO_TERMINATED,
    ...
    if (*re == NULL) {
        ...
        logMsg(BP_ERROR_LOG "Malformed tug livery config %s: regex compile "
...
tug_info_liv_cfg_match(const char *info_pathname, const char *icao,
    ...
    *force_pick_this = B_FALSE;
    ...
    if (fp == NULL) {
        logMsg(BP_ERROR_LOG "Error reading tug livery %s: cannot open info.cfg: %s",
        ...
        if (strcmp(cmd, "icao") == 0) {
            ...
            if (regex_str == NULL || *regex_str == 0 ||
                !regex_setup(regex_str, info_pathname, &re,
                             &match_data, 0)) {
                logMsg(BP_ERROR_LOG "Malformed tug livery config %s: bad "
            ...
            if (rc >= 0) {
                if (match_debug) {
                    logMsg(BP_WARN_LOG "regex \"%s\" "
                           "matches airport \"%s\"",
                           regex_str, icao);
                }
                icao_matched = B_TRUE;
            } else if (match_debug) {
                logMsg(BP_WARN_LOG "regex \"%s\" DOESN'T "
            ...
            *force_pick_this = B_TRUE;
            logMsg(BP_WARN_LOG "%s: present forcibly picking",
                   info_pathname);
        } else {
            logMsg(BP_ERROR_LOG "Malformed tug livery config %s: unknown "
...
tug_info_read(const char *tugdir, const char *tug_name, const char *icao,
    ...
    if (fp == NULL) {
        logMsg(BP_ERROR_LOG "Malformed tug %s, cannot open info.cfg: %s",
...
#define    READ_FILENAME(optname, result) \
    do { \
        ...
        if (fscanf(fp, "%255s", arg) != 1) { \
            logMsg(BP_ERROR_LOG "Malformed tug config file %s: expected " \
        ...
        if (!file_exists((result), &isdir) || isdir) { \
            logMsg(BP_ERROR_LOG "Malformed tug config file %s: '%s' not " \
...
#define    READ_NUMBER(fmt, optname, result) \
    do { \
        ...
        if (strcmp(option, "tug_obj") == 0) {
            ...
            logMsg(BP_INFO_LOG "Animation debugging active on %s", tugdir);
            ti->anim_debug = B_TRUE;
        } else if (strcmp(option, "drive_debug") == 0) {
            logMsg(BP_INFO_LOG "Driving debugging active on %s", tugdir);
            ti->drive_debug = B_TRUE;
        } else if (strcmp(option, "quick_debug") == 0) {
            logMsg(BP_INFO_LOG "Quick test debugging active on %s", tugdir);
            ...
            if (strcmp(type, "front") == 0) {
                ...
                logMsg(BP_ERROR_LOG "Malformed tug config file %s: bad "
            ...
            logMsg(BP_ERROR_LOG "Malformed tug config file %s: unknown "
...
#define    VALIDATE_TUG(cond, optname) \
    do { \
        if (cond) { \
            logMsg(BP_ERROR_LOG "Malformed tug config file %s: missing " \
...
#define    VALIDATE_TUG_REAL_NAN(field, optname) \
    ...
    if (ti->min_mtow >= ti->max_mtow) {
        logMsg(BP_ERROR_LOG "Malformed tug config file %s: min_mtow >= max_mtow",
               cfgfilename);
        goto errout;
    }
...
tug_info_select(double mtow, double ng_len, double tirrad, unsigned gear_type,
    ...
    if (dirp == NULL) {
        ...
        logMsg(BP_ERROR_LOG "Error reading directory %s: %s", tugdir,
               strerror(errno));
#endif    /* !IBM */
        free(tugdir);
        return (NULL);
...
reload_tug_handler(XPLMCommandRef cmd, XPLMCommandPhase phase, void *refcon) {
    ...
    if (glob_tug != NULL && !glob_tug->load_in_prog) {
        ...
        if (objpath == NULL) {
            logMsg(BP_ERROR_LOG "Error preparing tug object %s",
                   glob_tug->info->tug);
            return (1);
        }
...
tug_liv_apply(const tug_info_t *ti) {
    ...
    if (inobj == NULL) {
        logMsg(BP_ERROR_LOG "Cannot open tug object %s: %s", ti->tug,
               strerror(errno));
        goto errout;
    }
    outobj = fopen(objpath, "w");
    if (outobj == NULL) {
        logMsg(BP_ERROR_LOG "Cannot open tug object for writing %s: %s", objpath,
               strerror(errno));
        goto errout;
    }
...
tug_set_lift_arm_pos(const tug_t *tug, float x, bool_t grabbing_tire) {
    ...
    if (grabbing_tire) {
        ...
        double xx = (tug->tirrad - ti->min_tirrad) / (ti->max_tirrad - ti->min_tirrad);
        x = MIN(MAX(xx, 0), 1);
